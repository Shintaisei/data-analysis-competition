...
- **最高（クイック提出まで反映後）**: **movie_title_info + PCA 16 + critic_te × genre_Documentary（doc_x_critic_te）** → **Public 0.75493**（§4 クイック提出で更新）。
- それまでの最高: **movie_title_info（small）+ PCA 16** → **Public 0.75449**
- 映画情報のみ（movie_info）より、**タイトル＋映画情報の small 版**が Public で良い。
...
## 4. クイック提出（残り回数用・次元削減は PCA のみ）

**重い次元削減（ICA / UMAP 等）は使わず**、ドキュメントで「まだ試せてない」案だけを **PCA 16 固定**で提出する用。

**実行**  
- **ノートブック**: `quick_embedding_submissions.ipynb` を上から実行 → **11 本**（ベース 1 本 + バリエーション 10 本）が `outputs/submissions/` に保存される。提出回数 10 回なら好きな 10 本を選んで提出。  
- **スクリプト**: `python run_quick_embedding_submissions.py`（オプション: `--skip-existing`）

**ベース**: **movie_title_info + PCA 16**（Public 0.75449）。本番ベースライン（超える目標）は **doc_x_critic_te** の **Public 0.75493**。いずれもこの設定を土台にしている。

| 提出ファイル | 内容（いずれも docs 由来） |
|-------------|---------------------------|
| pca16（ベース） | 上記 0.75449 と同じ 1 本。ensemble_two 用にも作成。 |
| seed_avg | §4: シード 42,43,44 で 3 本学習 → 予測を平均 |
| pca24 | §4: PCA **24** 次元で 1 本 |
| large_pca16 | movie_title_info_large + PCA 16 の単体 1 本 |
| ensemble_two | §4: pca16 と large_pca16 を 0.5:0.5 で平均 |
| publisher_freq | 01_PREPROCESS: ベース + **publisher_name 出現回数** 1 列（CV +0.0003） |
| lda_10 | 01/03: ベース + movie_info の **LDA 10 次元**（CV +0.0006） |
| doc_x_critic_te | 02: ベース + **critic_te × genre_Documentary** 1 列（CV ベスト単体 +0.0010） |
| pca32 | PCA **32** 次元で 1 本 |
| pca8 | PCA **8** 次元で 1 本（次元を下げた版） |
| blend_pca16_pca24 | pca16 と pca24 の予測を 0.5:0.5 でブレンド |

処理は軽い（次元削減は PCA のみ）ので、残り提出回数でいろいろ試したいときに実行する。

---

### クイック提出の実験結果（Kaggle Public Score）

| 提出ファイル | Public Score |
|-------------|--------------|
| submission_embedding_movie_title_info_pca16_doc_x_critic_te.csv | **0.75493** |
| submission_embedding_movie_title_info_pca32.csv | 0.75465 |
| submission_embedding_movie_title_info_pca16_seed_avg.csv | 0.75483 |
| submission_embedding_ensemble_movie_title_info_and_large_pca16.csv | 0.75478 |
| submission_embedding_movie_title_info_pca16_publisher_freq.csv | 0.75472 |
| submission_embedding_movie_title_info_pca16_lda10.csv | 0.75348 |
| submission_embedding_movie_title_info_pca8.csv | 0.75437 |
| submission_embedding_movie_title_info_large_pca16.csv | 0.75440 |
| submission_embedding_movie_title_info_pca24.csv | 0.75423 |
| submission_embedding_movie_title_info_pca16.csv | 0.75449 |
| submission_embedding_movie_title_info_truncated_svd16.csv | 0.75389 |

→ **本番ベースライン（超える目標）**: **0.75493**。`train_baseline.ipynb` および今後の改善実験ではこのスコアをベースラインとする。
...
| ファイル | 役割 |
|----------|------|
| **`quick_embedding_submissions.ipynb`** | **ベース = movie_title_info + PCA 16。** 本番ベースライン（最高）は **doc_x_critic_te → Public 0.75493**。上から実行で 11 本（ベース + seed_avg, pca24, large_pca16, ensemble_two, publisher_freq, lda_10, doc_x_critic_te, pca32, pca8, blend_pca16_pca24）を一括作成。 |
| **`run_quick_embedding_submissions.py`** | 上記のスクリプト版（4 本。ノートブックで 8 本＋ベースを一括作成する想定）。 |
