...
train_text_vector_experiments.ipynb / text_vectors.py。ベース 34 特徴。時系列 CV。各 fold の tr のみでベクトル化を fit。ベース CV_AUC = **0.7600**。**hash_svd20 の途中で停止**。詳細は docs/TRAIN_TEXT_VECTOR_EXPERIMENTS_RESULTS.md。
...
### 6.7 hash_svd20 以降 — 要再開

hash_svd20, lda_10, lda_20, nmf_10, nmf_20, doc2vec_32, word2vec_32, sentence_transformer_32, concat_mi_title_tfidf_50, concat_mi_title_tfidf_svd20, concat_mi_title_count_50 は**未完了**。hash_svd20 の途中で停止したため、再開用セルから実行する。
